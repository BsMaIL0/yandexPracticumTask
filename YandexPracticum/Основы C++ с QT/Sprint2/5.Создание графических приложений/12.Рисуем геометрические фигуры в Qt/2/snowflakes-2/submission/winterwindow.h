...
public:
    ...
    void Draw(prac::QPainter& painter) const {
        ...
        painter.setPen(pen);
        for(int i = 0; i < 6; ++i) {
            Point2D end = DrawRotatedVector(painter, params_.center, params_.rotation + 60 * i, params_.size);
            if (params_.depth > 0) {
                GetNextLevelFlake(end).Draw(painter);  // Теперь 'end' объявлен
            }
        }
    }

    SnowFlake GetNextLevelFlake(Point2D new_center) const {
        // Порядок должен совпадать с объявлением структуры SnowFlakeParams:
        // center, color, size, line_width, rotation, depth, scale_factor
        return SnowFlake(SnowFlakeParams{
            new_center,                              // center
            params_.color,                           // color
            params_.size * params_.scale_factor,     // size
            params_.line_width,                      // line_width
            params_.rotation,                        // rotation
            params_.depth - 1,                       // depth
